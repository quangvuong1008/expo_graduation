...
const Transaction = memo(({ route }: any) => {
  ...
  const [tabLabelFormat, setTabLabelFormat] = useState<FrequenceDateFormat>(FrequenceDateFormat.month);
  ...
  useFocusEffect(
    React.useCallback(() => {
      if (route && route.params && route.params.wallet && route.params.wallet.id != -1) {
   ...
   *  Render tab content
   */
  const renderTransactionPage = useCallback((date: Moment, page: number) => {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        >
          {
            <>
              <Text style={styles.textTotalWallets}>{wallet?.name}</Text>
        ...
        {
          currency ?
            <Text style={styles.numberWallet}>
              {currencyFormat(wallet?.balance, currency)}
            </Text>
            : null
        }
      </View>
      <ScrollableTab titles={[
