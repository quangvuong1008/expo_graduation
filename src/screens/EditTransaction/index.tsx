...
import { View, StyleSheet, TextInput, Platform, Animated, Alert } from "react-native";
...
import { onDeleteTransactionRequest, onUpdateTransactionRequest } from "../../store/actions/transactionAction";
...
const EditTransaction = memo(({ route }: any) => {
    ...
    switch (tabActive) {
      case 0:
        if (wallet && category && date) {
          await dispatch(onUpdateTransactionRequest({
            id: transaction.id,
            balanceMinus: route.params?.type == "expense" ? wallet.balance + (balance - transaction.balance) : wallet.balance - (transaction.balance + balance),
            balanceAdd: 0,
            balance: balance,
      ...
      case 1:
        if (wallet && category && date) {
          await dispatch(onUpdateTransactionRequest({
            id: transaction.id,
            balanceMinus: 0,
            balanceAdd: route.params?.type == "expense" ? wallet.balance + (balance + transaction.balance) : wallet.balance + (balance - transaction.balance),
            balance: balance,
            categoryId: category.id,
            walletId: wallet.id,
            walletFromId: 0,
      ...
      case 2:
        if (walletFrom.id == walletTo.id) {
          ...
          if (wallet && category && date) {
            await dispatch(onUpdateTransactionRequest({
              id: transaction.id,
              balanceAdd: route.params?.type == "transfer" ? walletTo.balance + (balance - transaction.balance) : walletTo.balance + balance,
              balanceMinus: route.params?.type == "income" ? wallet.balance - (transaction.balance + balance) : walletFrom.balance + (balance - transaction.balance),
              balance: balance,
              categoryId: 52,
              walletId: 0,
  ...
  const onPressYes = async () => {
    await dispatch(onDeleteTransactionRequest(transaction, wallet?.balance || 0));
    navigation.goBack();
    setShowDialog(false);
  };
          ...
          tabActive == 2 ?
            ...
            : <View style={styles.contentView}>
              ...
              <AnimatedInput
                ...
                imageIcon={wallet?.typeWallet.icon ? IMAGE_ICON[`${wallet.typeWallet.icon}`] : null}
