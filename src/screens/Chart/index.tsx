...
import { ChartInMonth, IChartDatas } from "@store/models/reducers/chart";
...
const Chart = memo(({ route }: any) => {
  ...
  function setDataChartUI(data: Array<CHART>, incomArr: Array<number>, expenseArr: Array<number>) {
  ...
  const renderStatisticPage = useCallback((date: Moment) => {
    ...
    for (let i = 1; i <= date.daysInMonth(); i++) {
      category.push(`${month}/${i}`);
      incomeArr.push(0);
      expenseArr.push(0);
    }
    const dataUI: any = setDataChartUI(chartElement?.dailyChartData || [], incomeArr, expenseArr);
    return (
      <StatisticPage
        walletId={walletId}
            ...
            <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
              <View style={styles.header}>
                ...
                >
                  {
                    <>
                      <Text style={styles.textTotalWallets}>{walletName}</Text>
